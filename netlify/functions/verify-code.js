...
export const handler = async (event) => {
  ...
  if (!normalized || !code) {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Укажите номер телефона и код' }) };
  }

  // Найти актуальную запись кода
  ...
  // Неверный код — увеличить счётчик
  ...
  return {
    statusCode: 200,
    headers: CORS,
    body: JSON.stringify({ success: true, token, phone: normalized, userId }),
  };
};
