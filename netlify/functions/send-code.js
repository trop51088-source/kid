...
export const handler = async (event) => {
  ...
  if (!normalized) {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Неверный формат номера телефона' }) };
  }
  ...
  if (count >= 3) {
    return {
      statusCode: 429,
      headers: CORS,
      body: JSON.stringify({ error: 'Превышен лимит SMS. Попробуйте через час.' }),
    };
  }
  ...
  return {
    statusCode: 200,
    headers: CORS,
    body: JSON.stringify({ success: true, phone: normalized }),
  };
};
